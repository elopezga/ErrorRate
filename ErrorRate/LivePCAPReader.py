...
# Run a trial experiment based on number of packets to read
...
# Run trail experiment based on how long you want to read
...
def RunForErrors(numerrs, iface, filename):
    ...
    # Construct tshark call string
    tsharkArgs = " -i " + iface + " -w " + filename + " -F pcap";
    ...
    # Start tshark thread as daemon so that it exits when this function exits
    sharkthread = threading.Thread(name='tshark', target=os.system, args=("sudo tshark" + tsharkArgs));
    sharkthread.setDaemon(True);
    sharkthread.start();


    # Start analysis process
    pkts = rdpcap(filename);
    start = 0; # Start reading pcap from begining of file
    end = len(pkts)-1; # Stop reading at first open pcap instance

    while 1:
        while( start <= end ):
            # Updates metaData from previous call
            processPkt2(pkts[start], metaData);
            start += 1; # Required for looping

            # Real time data printing
            #metaData.printInfo();

            if (metaData.totPktsLost == numerrs):
                break;
